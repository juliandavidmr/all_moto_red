...
export default function Detail({ motorcycle }: MotorcycleProps) {
	...
	useEffect(() => {
		function sendView() {
			if (process.browser) {
				window.fetch('/api/motorcycles/view', {
					method: 'POST',
	...
	const fbLink = () => {
		return process.browser ? `https://www.facebook.com/sharer.php?u=${ location.href }` : ''
	}

	const twitLink = () => {
		return process.browser ? `https://twitter.com/intent/tweet?url=${ location.href }&text=${ 'Mira esta motocicleta ' + motorcycle.name }&hashtags=allmotored` : ''
	}
	...
	return (
		<>
			...
			<Layout>
				<section
					className="relative text-gray-600 body-font overflow-hidden bg-white lg:w-11/12 mx-auto flex flex-wrap shadow-md rounded-md duration-500">
						...
						<div className="flex mt-8">
							...
							<div className="ml-auto inline-flex">
								<Link href={ fbLink() } passHref>
									<a className="text-gray-500" target="_blank" rel="noopener noreferrer" title="Compartir en Facebook">
